...
class SequentialFlowC(nn.Module):
    """A generalized nn.Sequential container for normalizing flows.
    """

    def __init__(self, layersList):
        super(SequentialFlowC, self).__init__()
        self.chain = nn.ModuleList(layersList)
    ...
    def forward(self, x, context, logpx=None, reverse=False, inds=None):
        ...
        if logpx is None:
            for i in inds:
                x = self.chain[i](x, context, reverse=reverse)
            return x
        else:
            for i in inds:
                x, logpx = self.chain[i](x, context, logpx, reverse=reverse)
            return x, logpx