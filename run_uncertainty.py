...
from training.io_utils import model_dict, parse_args, get_best_file, get_assigned_file
    ...
    if params.method in ['maml', 'maml_approx', 'DKT']:  # maml do not support testing with feature
        ...
        for i in range(iter_num):
            ...
            for cl in select_class:
                ...
                z_all.append(
                    [np.squeeze(img_feat[perm_ids[i]]) for i in range(n_support + n_query)])  # stack each batch
...
def main():
    ...
    if (len(temperature_list) > 0):
        temperature = np.mean(temperature_list)
    else:
        temperature = 1.0
